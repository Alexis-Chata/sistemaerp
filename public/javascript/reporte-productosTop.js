...
//		}
                ...
                consultarTablaProductosTop();
		
	});
	$('#btnImprimir').click(function(e){
		e.preventDefault();
		imprSelec('contenidoProductosTop');
	});
        
        $('#btnConsultarExcel').click(function(e){
		if(confirm("¿Desea exportar en excel?")){
                    $('#frmProductosTop').attr('action','/reporte/exportarExcelProductosTop');
                }else{
                    e.preventDefault();
                }
	});
...
function consultarTablaProductosTop(){
    $('#contenidoProductosTop').html("<center><img src='/public/imagenes/cargando.gif' width='300'></center>");
        var linea = $('#lstLinea').val();
        var marca = $('#lstMarca').val();
        var finicio = $('#fechaInicio').val();
        var ffinal = $('#fechaFinal').val();
        //console.log(linea+'a'+marca+'b'+finicio+'c'+ffinal);
        //console.log('3');
        var parametros = {
            linea:linea,
            marca:marca,
        ...
        $.ajax({
		url:'/reporte/listaProductosTop',
                data: parametros,
		type:'post',
		dataType:'html',
		success:function(data){
                        //console.log(data);
			$('#contenidoProductosTop').html(data);
		}
	});
}